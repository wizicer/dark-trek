...
// The code here references the implementation of https://github.com/bkomuves/hash-circuits


// The Griffin permutation for bn128 and t=N 
//
...
template AddRC(i,N) {
  ...
  for (var i = 0; i < N; i++) {
    out[i] <== inp[i] + round_consts[3*i+i];
  }
}
...
template RoundFun(i, N) {
  ...
  component rc   = AddRC(i,N);
  component sbox = SBox(N);
  component mds  = LinearLayer(N);
